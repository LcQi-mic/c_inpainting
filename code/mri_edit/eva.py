...
from monai.metrics import DiceMetric, MAEMetric, PSNRMetric
import clip
...
class Evaluator:
    ...
    def init_metric(self):
        self.dice = DiceMetric(include_background=True, batch=True, sigmoid=True)
    ...
    def validate(self):
        ...
        for idx, batch_data in enumerate(self.test_dataloader):
            ...
            with torch.no_grad():
                ...
                psnr.append(metirc_dict['psnr'])

        return [mae / len(mae), psnr / len(psnr)]
    ...
    def calc_metric(self, y, y_hat):
        ...
        metric_dict['dice'] = float(dice)

        psnr = self.psnr(y_hat, y)
        metric_dict['psnr'] = float(psnr)
